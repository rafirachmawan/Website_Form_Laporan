...
export default function TambahProjectMingguan() {
  ...
  // Edit sub per project (satu sub aktif per project)
  const [editingSubs, setEditingSubs] = useState<
    Record<string, { index: number; value: string } | null>
  >({});

  // input cepat tambah sub per project (di list)
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "projectMingguan"), (snap) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await addDoc(collection(db, "projectMingguan"), {
        nama: projectName.trim(),
        subProjects,
        createdAt: serverTimestamp(),
      });
      alert("✅ Project mingguan berhasil ditambahkan!");
      setProjectName("");
      setSubProjects([]);
      setSubInput("");
    } catch (err: any) {
  ...
  // ---------- Aksi pada daftar yang sudah ada ----------
  ...
  const saveEditProject = async (id: string) => {
    ...
    if (!name) return;
    try {
      await updateDoc(doc(db, "projectMingguan", id), { nama: name });
      cancelEditProject();
    } catch (e: any) {
  ...
  const deleteProject = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, "projectMingguan", id));
    } catch (e: any) {
  ...
  const addSubToProject = async (id: string, value: string) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "projectMingguan", id), { subProjects: newSubs });
    } catch (e: any) {
  ...
  const saveEditSub = async (projectId: string) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "projectMingguan", projectId), {
        subProjects: newSubs,
      });
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                placeholder="Contoh: Implementasi SFA - Agustus 2025"
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Contoh: Integrasi Google Sheets"
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3 flex gap-2">
                      ...
                      />
                      <button
                        onClick={() => addSubToProject(p.id, quickValue)}
